...
class Client {
 public:
  ...
  /* [239 新增] 构建删除公钥消息 — 从白名单删除指定公钥
   *   对应官方 SDK remove-key 命令, 使用 WhitelistOperation.removePublicKeyFromWhitelist */
  int build_remove_key_message(const pb_byte_t *public_key, size_t public_key_size,
                               pb_byte_t *output_buffer, size_t *output_length);

  /* [239 新增] 构建添加指定公钥消息 — 向白名单添加外部公钥(非自己生成的)
   *   对应官方 SDK add-key 命令, 使用 WhitelistOperation.addKeyToWhitelistAndAddPermissions
   *   与 build_white_list_message 不同: 后者用自己生成的公钥, 此函数接受外部公钥 */
  int build_add_key_message(const pb_byte_t *public_key, size_t public_key_size,
